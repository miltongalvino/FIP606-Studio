...
ui <- page_navbar(
  ...
  nav_panel(title=tags$span("Início"), value="aba_inicio",
    ...
    tags$div(class="rendr-container",
      ...
      tags$p(class="rendr-subtitle", "Faça o upload da sua planilha (CSV ou Excel) e inicie suas análises e visualizações."),
      ...
      tags$p(style="text-align: center; font-size: 0.78rem; color: #9AA0A6; margin-top: 5px; margin-bottom: 0;",
        tags$a(href="https://github.com/miltongalvino/FIP606-Studio", target="_blank", style="color: #9AA0A6; text-decoration: none;", 
               icon("github"), "Acesso ao projeto no GitHub"))
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$dl_report <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Garantir que a pasta www e as imagens estejam no tempdir para o pandoc encontrar
      temp_www <- file.path(tempdir(), "www")
      if (!dir.exists(temp_www)) dir.create(temp_www, showWarnings = FALSE)
      app_www <- file.path(app_dir, "www")
      if (file.exists(file.path(app_www, "report_icon.png"))) file.copy(file.path(app_www, "report_icon.png"), file.path(temp_www, "report_icon.png"), overwrite = TRUE)
      if (file.exists(file.path(app_www, "app_icon.png"))) file.copy(file.path(app_www, "app_icon.png"), file.path(temp_www, "app_icon.png"), overwrite = TRUE)
  ...
  # --- UI Suggestion logic ---
  output$ui_analysis_suggestion <- renderUI({
    req(rv$data)
    d <- rv$data
    
    num_cols <- sum(sapply(d, is.numeric))
    cat_cols <- sum(sapply(d, function(x) is.character(x) || is.factor(x)))
    
    if (num_cols >= 2 && cat_cols == 0) {
      sug_text <- "Parece que você tem várias variáveis numéricas e nenhuma categórica. A <b>Correlação</b> ou <b>Regressão</b> pode ser o melhor ponto de partida."
      btn_id <- "btn_sug_cor"
      btn_label <- "Ir para Correlação"
      btn_icon <- "project-diagram"
    } else if (num_cols >= 1 && cat_cols == 1) {
      cat_col_name <- names(d)[sapply(d, function(x) is.character(x) || is.factor(x))][1]
      lvl_count <- length(unique(d[[cat_col_name]]))
      if (lvl_count == 2) {
        sug_text <- "Você tem uma resposta numérica e um fator com exatamente 2 níveis. O <b>Teste T</b> (ou Wilcoxon) é a escolha clássica para comparar esses dois grupos."
        btn_id <- "btn_sug_ttest"
        btn_label <- "Ir para Teste T"
        btn_icon <- "scale-balanced"
      } else {
        sug_text <- "Você tem uma resposta numérica e um fator com mais de 2 grupos. A <b>ANOVA</b> é sugerida para verificar diferenças nas médias."
        btn_id <- "btn_sug_anova"
        btn_label <- "Ir para ANOVA"
        btn_icon <- "layer-group"
      }
    } else if (num_cols >= 1 && cat_cols >= 2) {
      sug_text <- "Você possui dados com múltiplos fatores/grupos. Sugerimos a <b>ANOVA</b> (Fatorial ou Blocos) para analisar as médias e possíveis interações."
      btn_id <- "btn_sug_anova"
      btn_label <- "Ir para ANOVA"
      btn_icon <- "layer-group"
    } else {
      sug_text <- "Recomendamos que você inicie pela <b>Análise Exploratória de Dados (AED)</b> para entender melhor o comportamento dos seus dados."
      btn_id <- "btn_sug_aed"
      btn_label <- "Ir para AED"
      btn_icon <- "magnifying-glass-chart"
    }
    
    div(class = "alert alert-dismissible fade show shadow-lg", 
        style = "position: fixed; top: 80px; right: 30px; width: 420px; z-index: 1050; border-left: 5px solid #064E3B; background-color: #F0FDF4; color: #064E3B; border-radius: 12px;",
      tags$button(type = "button", class = "btn-close", `data-bs-dismiss` = "alert", `aria-label` = "Close"),
      tags$button(type = "button", style = "position: absolute; top: 12px; right: 45px; z-index: 2; background: transparent; border: none; color: #064E3B; opacity: 0.6; padding: 0.5rem;", `data-bs-toggle` = "collapse", `data-bs-target` = "#sug_body", icon("minus")),
      h5(icon("lightbulb"), " Sugestão de Análise", style = "font-weight: 800; margin-bottom: 10px; font-size: 1.15rem; color: #064E3B;"),
      div(id = "sug_body", class = "collapse show",
        p(HTML(sug_text), style = "font-size: 0.95rem; margin-bottom: 15px; line-height: 1.4;"),
        actionButton(btn_id, btn_label, class = "btn-success btn-sm w-100", style="font-weight: 600; padding: 8px;", icon = icon(btn_icon))
      )
    )
  })
  
  observeEvent(input$btn_sug_cor, { 
    nav_select("main_nav", "aba_cor")
    if(!is.null(rv$data)) {
      num_cols <- names(rv$data)[sapply(rv$data, is.numeric)]
      if(length(num_cols) >= 2) {
        updateSelectInput(session, "cor_cols", selected = num_cols)
        rv$cor_cols <- num_cols
      }
    }
  })
  
  observeEvent(input$btn_sug_ttest, { 
    nav_select("main_nav", "aba_ttest")
    if(!is.null(rv$data)) {
      num_cols <- names(rv$data)[sapply(rv$data, is.numeric)]
      cat_cols <- names(rv$data)[sapply(rv$data, function(x) is.character(x) || is.factor(x))]
      if(length(cat_cols) > 0) {
        updateSelectInput(session, "ttest_group", selected = cat_cols[1])
        rv$ttest_group <- cat_cols[1]
      }
      if(length(num_cols) > 0) {
        updateSelectInput(session, "ttest_resp", selected = num_cols[1])
        rv$ttest_resp <- num_cols[1]
      }
    }
  })
  
  observeEvent(input$btn_sug_anova, { 
    nav_select("main_nav", "aba_anova")
    if(!is.null(rv$data)) {
      num_cols <- names(rv$data)[sapply(rv$data, is.numeric)]
      cat_cols <- names(rv$data)[sapply(rv$data, function(x) is.character(x) || is.factor(x))]
      if(length(cat_cols) > 0) {
        updateSelectInput(session, "anova_trat1", selected = cat_cols[1])
        rv$anova_trat1 <- cat_cols[1]
      }
      if(length(cat_cols) >= 2) {
        updateSelectInput(session, "anova_trat2", selected = cat_cols[2])
        rv$anova_trat2 <- cat_cols[2]
      }
      if(length(cat_cols) >= 3) {
        updateSelectInput(session, "anova_bloco", selected = cat_cols[3])
        rv$anova_bloco <- cat_cols[3]
      }
      if(length(num_cols) > 0) {
        updateSelectInput(session, "anova_resp", selected = num_cols[1])
        rv$anova_resp <- num_cols[1]
      }
    }
  })
  
  observeEvent(input$btn_sug_aed, { 
    nav_select("main_nav", "aba_aed")
    if(!is.null(rv$data)) {
      num_cols <- names(rv$data)[sapply(rv$data, is.numeric)]
      cat_cols <- names(rv$data)[sapply(rv$data, function(x) is.character(x) || is.factor(x))]
      if(length(cat_cols) > 0) {
        updateSelectInput(session, "aed_trat1", selected = cat_cols[1])
        rv$aed_trat1 <- cat_cols[1]
      }
      if(length(cat_cols) >= 2) {
        updateSelectInput(session, "aed_trat2", selected = cat_cols[2])
        rv$aed_trat2 <- cat_cols[2]
      }
      if(length(cat_cols) >= 3) {
        updateSelectInput(session, "aed_bloco", selected = cat_cols[3])
        rv$aed_bloco <- cat_cols[3]
      }
      if(length(num_cols) > 0) {
        updateSelectInput(session, "aed_resp", selected = num_cols[1])
        rv$aed_resp <- num_cols[1]
      }
    }
  })
